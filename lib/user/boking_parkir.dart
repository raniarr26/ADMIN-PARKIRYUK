...
class _BookingHistoryPageState extends State<BookingHistoryPage> {
  final Set<String> _processedBookings = {}; // To prevent redundant updates
  ...
  Duration _calculateRemainingTimeFromSlot(String timeSlot, String bookingDate) {
    try {
      DateTime parsedDate = DateFormat('yyyy-MM-dd').parse(bookingDate);
      ...
      debugPrint('Error calculating remaining time for $bookingDate $timeSlot: $e');
      return Duration.zero;
    }
  }
  ...
  Duration _calculateUsedTimeFromSlot(String timeSlot, String bookingDate) {
    try {
      DateTime parsedDate = DateFormat('yyyy-MM-dd').parse(bookingDate);
      ...
      debugPrint('Error calculating used time for $bookingDate $timeSlot: $e');
      return Duration.zero;
    }
  }

  // Fungsi untuk mengecek apakah slot sudah dimulai
  bool _hasSlotStarted(String timeSlot, String bookingDate) {
    try {
      DateTime parsedDate = DateFormat('yyyy-MM-dd').parse(bookingDate);
      String startTimeStr = timeSlot.split('-')[0].trim();
      ...
      debugPrint('Error checking if slot has started for $bookingDate $timeSlot: $e');
      return false;
    }
  }

  // Fungsi untuk mengecek apakah slot sudah berakhir
  bool _hasSlotExpired(String timeSlot, String bookingDate) {
    try {
      DateTime parsedDate = DateFormat('yyyy-MM-dd').parse(bookingDate);
      String endTime = timeSlot.split('-')[1].trim();
      List<String> timeParts = endTime.split(':');
      ...
      debugPrint('Error checking if slot has expired for $bookingDate $timeSlot: $e');
      return false;
    }
  }

  Future<void> _updateParkingGateStatus(String gateName, String slotNumber, String bookingId) async {
    if (_processedBookings.contains(bookingId)) {
      debugPrint('Booking $bookingId already processed. Skipping update.');
      return;
    }

    try {
      _processedBookings.add(bookingId);
      ...
      if (gateQuery.docs.isNotEmpty) {
        ...
        debugPrint('Status parking gate $gateName-$slotNumber (via booking $bookingId) berhasil diupdate menjadi kosong');
        debugPrint('Booking $bookingId status updated to expired');
      } else {
        debugPrint('Parking gate document for $gateName not found for booking $bookingId.');
      }
    } catch (e) {
      debugPrint('Error updating parking gate status for booking $bookingId: $e');
      _processedBookings.remove(bookingId);
    }
  }

  void _checkAndUpdateExpiredBookings(List<DocumentSnapshot> docs) {
    // Hanya cek booking untuk tanggal hari ini
    ...
    for (DocumentSnapshot doc in docs) {
      ...
      if ((status.toLowerCase() == 'active' || status.toLowerCase() == 'confirmed') &&
          ...
          !_processedBookings.contains(bookingId)) {
        
        // Cek apakah slot sudah expired menggunakan fungsi yang diperbaiki
        bool isExpired = _hasSlotExpired(timeSlot, bookingDate);

        if (isExpired) {
          _updateParkingGateStatus(gateName, slotNumber, bookingId);
        }
      }
    }
  }
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<DocumentSnapshot> bookings = List.from(snapshot.data!.docs);
          bookings.sort((a, b) {
          ...
          _checkAndUpdateExpiredBookings(bookings);

          return ListView.builder(
            padding: const EdgeInsets.all(16),
            itemCount: bookings.length,
            itemBuilder: (context, index) {
              DocumentSnapshot doc = bookings[index];
              Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
              ...
              return BookingCard(
                ...
                data: data,
                bookingId: doc.id,
                onUpdateStatus: (gateName, slotNumber, bookingId) =>
                    _updateParkingGateStatus(gateName, slotNumber, bookingId),
...
// BookingCard dengan logika penghitungan yang diperbaiki
...
class _BookingCardState extends State<BookingCard> {
  ...
  void _updateTimes() {
    final String bookingDate = widget.data['booking_date'] ?? '';
    final String timeSlot = widget.data['time_slot'] ?? '';

    _remainingTime = widget.calculateRemainingTimeFromSlot(timeSlot, bookingDate);
    _usedTime = widget.calculateUsedTimeFromSlot(timeSlot, bookingDate);
    _isExpired = widget.hasSlotExpired(timeSlot, bookingDate);
    _hasStarted = widget.hasSlotStarted(timeSlot, bookingDate);
  }

  void _startCardTimer() {
    String status = widget.data['status'] ?? '';
    
    // Timer hanya jalan jika booking masih aktif/confirmed dan belum expired
  ...
  @override
  Widget build(BuildContext context) {
    String bookingCode = widget.data['booking_code'] ?? 'N/A';
    String bookingDate = widget.data['booking_date'] ?? '';
              ...
              children: [
                Expanded(
                  child: _buildDetailItem(
                    ...
                    'Tanggal',
                    DateFormat('dd MMMM yyyy').format(DateTime.parse(bookingDate)),
                  ),
                ),
                Expanded(
                  child: _buildDetailItem(
